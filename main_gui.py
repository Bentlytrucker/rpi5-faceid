...
class FaceRecognitionManager:
    ...
    def process_frame(self, frame):
        ...
        if not face_box:
            self.recognition_start_time = None; return frame, None, None
        x, y, w, h = face_box; face_roi = frame[y:y+h, x:x+w]
        current_embedding = self.get_face_embedding(face_roi)
        if current_embedding is None: return frame, face_box, None
        ...
        if best_match and best_sim > SIMILARITY_THRESHOLD:
            if self.current_recognized_face != best_match:
                self.current_recognized_face = best_match; self.recognition_start_time = time.time()
            ...
            self.current_recognized_face = None; self.recognition_start_time = None
